...
- **Programmi Riscaldamento** — sequenza di sessioni a corpo libero (N esercizi + round ciascuna), riusabile su più atleti — stessa forma/UX di Forza e Circuito, ma senza un ordine fisso da rispettare: un Programma Riscaldamento assegnato propone ogni volta una sessione **scelta a caso** tra tutte le sue (mai la stessa appena vista, se ce n'è più di una), e non "finisce" mai. L'atleta può anche chiederne subito un'altra ("Cambia riscaldamento") senza che questo conti come fatta — sempre pescata a caso.
...
- **Assegnazione per atleta** — Riscaldamento, Forza e Circuito si assegnano **separatamente**, ciascuno con un menu a tendina (un Programma solo per sezione), dal pannello Atleti. Ognuna delle 3 sezioni ha la propria posizione di avanzamento: completare la Forza non tocca il Circuito né il Riscaldamento. Forza e Circuito mostrano **fino a 3 sessioni tra cui scegliere**, in coda (vedi sotto); il Riscaldamento propone ogni volta una sessione **a caso**, sempre disponibile, mai "finisce". Le sessioni non sono righe salvate: si **assemblano al volo** dalla libreria assegnata, alla posizione corrente dell'atleta.
- **Player** — a schermo intero, GIF di sfondo, controlli play/pausa/avanti/indietro, annunci vocali (Web Speech API del browser o ElevenLabs, vedi sotto). Countdown per esercizi/serie; il **riposo tra blocchi è un cronometro che conta in su** (nessun tempo imposto: l'atleta preme avanti quando è pronto, vedendo quanto tempo è passato) e mostra un'anteprima del blocco in arrivo (solo Circuiti). Il livello di carico da usare (per esercizi con attrezzo, corpo libero escluso) lo **prescrivi tu** in fase di creazione — l'atleta lo vede a schermo insieme a nome esercizio e ripetizioni, non gli viene chiesto. Una serie Max/AMRAP chiede ancora quante ripetizioni sono state fatte solo in **Forza** (conta come dato di progressione nel tempo); in Circuito/Riscaldamento un Max (es. Mountain Climber, Burpees, Plank) non lo chiede più — era un'interruzione non voluta su ogni esercizio Max, non solo sui pochi dove il conteggio serve davvero. L'altra cosa annotata dall'atleta durante l'esecuzione è il peso davvero usato, a fine di ogni blocco Circuito con attrezzi (vedi sotto).
...
supabase/
  ...
  migration_random_warmup.sql       idem, dopo la precedente — riscaldamento a caso invece che numerato, con "Cambia"
  migration_access_expiry.sql       idem, dopo la precedente — scadenza opzionale dell'accesso per atleta
scripts/
...
- **Service Worker** (`vite-plugin-pwa`, configurato in `vite.config.js`): attivo solo nella build di produzione (non in `npm run dev`). I video/GIF esercizio (Supabase Storage e gli eventuali link Google Drive/Photos rimasti) si cachano cache-first per 60 giorni — la prima volta che un esercizio si vede scarica, le volte dopo è istantaneo, anche offline.
- **Aggiornamento dell'app**: il browser ricontrolla se c'è una versione nuova solo con un vero nuovo caricamento di pagina — su iOS, riaprire l'app dalla schermata Home spesso *riprende* il processo sospeso invece di ricaricare davvero, quindi quel controllo può non scattare mai da solo e l'app resta bloccata su una versione vecchia (il caso "ho pushato ma l'utente vede ancora quella di prima" anche con `registerType: "autoUpdate"`). Per questo la registrazione del Service Worker è a mano (`src/main.jsx`, `injectRegister: false` in `vite.config.js` per disattivare quella iniettata di default, che registra una volta e basta): ricontrolla esplicitamente quando l'app torna visibile e ogni 30 minuti mentre resta aperta, e ricarica la pagina da sola quando trova un aggiornamento — nessuna azione richiesta all'atleta, niente più "svuota cache manualmente".
...
- **Solo nei Circuiti** (`workout.kind === "circuit"`, impostato in `getCircuitChoices`): un bip generato al volo (Web Audio API, nessun file audio) nei 3 secondi prima del cambio esercizio, per sentire "si cambia" senza aspettare la voce; durante il riposo tra blocchi, un'anteprima (foto/video + nome + carico) degli esercizi del blocco in arrivo, per organizzare il cambio di attrezzi mentre si aspetta.
...
- Ogni atleta ha 3 assegnazioni indipendenti: Riscaldamento (`assigned_warmup_program_id`+`warmup_position`) propone una sessione scelta **a caso** (mai la stessa appena vista), sempre disponibile — "Cambia riscaldamento" (`reroll_warmup`) ne pesca subito un'altra senza contare come fatta. Forza e Circuito (`assigned_strength_program_id`/`assigned_circuit_program_id` + `strength_queue`/`circuit_queue` + `strength_batch_size`/`circuit_batch_size`) mostrano invece **fino a 3 sessioni tra cui l'atleta sceglie** — farne una la completa per sempre, scartarla la rimanda in fondo alla coda (non sparisce, il totale non cambia); solo quando tutte e 3 sono risolte ne compaiono altre 3. L'admin può anche far partire un atleta da una sessione specifica (non necessariamente la prima) — utile per chi arriva a metà ciclo da un altro progetto. Le sessioni si generano al volo dalla libreria assegnata, non sono righe salvate: eliminare/modificare un Programma assegnato cambia cosa vede l'atleta la prossima volta.
- **Scadenza accesso** (`profiles.access_until`, opzionale — per clienti che acquistano un mese o una prova su un programma che dura più mesi): impostata dall'admin nella card atleta (nessuna data = accesso illimitato, comportamento di sempre). Blocco solo lato app, non a livello di database: superata la data, `AthleteHome` mostra un messaggio invece del menu allenamenti — si sblocca da solo spostando la data in avanti, nessuna azione richiesta all'atleta.
