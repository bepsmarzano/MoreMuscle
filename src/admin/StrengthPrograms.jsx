...
import { StrengthBlockEditor } from "./blockEditors.jsx";
import * as api from "../lib/api.js";

// ---------------------------------------------------------------------------
// Programmi Forza: sequenza ORDINATA di sessioni Forza (progressione),
// riusabile su più piani/atleti. Ogni sessione = un blocco Forza di oggi
// (stesso esercizio, riscaldamento specifico + serie di lavoro).
// ---------------------------------------------------------------------------
export default function StrengthPrograms({ library }) {
  ...
  const load = async () => {
    setLoading(true);
    try {
      setPrograms(await api.listStrengthPrograms());
      setError("");
    } catch (e) {
  ...
  const save = async (program) => {
    try {
      await api.saveStrengthProgram(program);
      setEditing(null);
      load();
    } catch (e) {
  ...
  const remove = async (id) => {
    try {
      await api.deleteStrengthProgram(id);
  ...
  return (
    <div>
      <div style={S.sectionRow}>
        <div>
          <h2 style={S.h2}>Programmi Forza</h2>
          <p style={S.muted}>Sequenze di sessioni Forza (progressione) riusabili su più piani/atleti</p>
        </div>
    ...
    <div style={S.modalWrap} onClick={onCancel}>
      <div style={S.modal} onClick={(e) => e.stopPropagation()}>
        <div style={S.modalHead}>
          <span style={S.modalTitle}>Programma Forza</span>
          <button style={S.iconBtnSm} onClick={onCancel}><X size={15} /></button>
        </div>
        <label style={S.fieldLbl}>Nome</label>
        <input style={S.fieldInput} value={name} onChange={(e) => setName(e.target.value)} placeholder="Es. Squat — 8 settimane" />
        ...
        {sessions.map((s, i) => (
          <div key={i} style={S.blockCard}>
            <div style={S.blockHead}>
              ...
              <span style={S.muted}>{s.exerciseName || "senza esercizio"}</span>
            ...
            {openIndex === i && <StrengthBlockEditor block={s} onPatch={(patch) => patchSession(i, patch)} library={library} />}
          </div>
        ))}
